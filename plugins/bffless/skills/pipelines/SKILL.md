...
| **Video (ffmpeg)** | `ffmpeg_handler` | Server-side video ops on storage objects: probe, extract_audio, slice, concat, frames, contact_sheet. Opt-in per instance; probe first and fall back to the browser on `server: false` |
